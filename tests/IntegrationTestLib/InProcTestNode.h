...
public:
  InProcTestNode(const TestNodeConfiguration& cfg, const Quid::Currency& currency);
  ~InProcTestNode();
  ...
  virtual bool getBlockTemplate(const std::string &minerAddress, Quid::Block &blockTemplate, uint64_t &difficulty) override;
  virtual bool submitBlock(const std::string& block) override;
  virtual bool getTailBlockId(Crypto::Hash &tailBlockId) override;
  virtual bool makeINode(std::unique_ptr<Quid::INode>& node) override;
...
private:
  ...
  std::unique_ptr<Quid::core> core;
  std::unique_ptr<Quid::QuidProtocolHandler> protocol;
  std::unique_ptr<Quid::NodeServer> p2pNode;
