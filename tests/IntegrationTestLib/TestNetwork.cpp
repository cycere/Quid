...
#ifdef _WIN32
const std::string daemonExec = std::string(Quid::QUID_NAME) + "d.exe";
#else
const std::string daemonExec = std::string(Quid::QUID_NAME) + "d";
#endif
...
TestNodeConfiguration TestNetworkBuilder::buildNodeConfiguration(size_t index) {
  ...
  cfg.testnet = testnet;
  cfg.logFile = std::string("test_") + Quid::QUID_NAME + "d" + std::to_string(index) + ".log";
...
TestNetwork::TestNetwork(System::Dispatcher& dispatcher, const Quid::Currency& currency) : 
