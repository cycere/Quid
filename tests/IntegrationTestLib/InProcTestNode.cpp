...
InProcTestNode::InProcTestNode(const TestNodeConfiguration& cfg, const Quid::Currency& currency) : 
...
void InProcTestNode::workerThread(std::promise<std::string>& initPromise) {
  ...
  try {
    ...
    core.reset(new Quid::core(m_currency, NULL, log));
    protocol.reset(new Quid::QuidProtocolHandler(m_currency, dispatcher, *core, NULL, log));
    p2pNode.reset(new Quid::NodeServer(dispatcher, *protocol, log));
    protocol->set_p2p_endpoint(p2pNode.get());
    core->set_cryptonote_protocol(protocol.get());

    Quid::NetNodeConfig p2pConfig;
...
bool InProcTestNode::getBlockTemplate(const std::string &minerAddress, Quid::Block &blockTemplate, uint64_t &difficulty) {
  AccountPublicAddress addr;
...
bool InProcTestNode::makeINode(std::unique_ptr<Quid::INode> &node) {

  std::unique_ptr<InProcessNode> inprocNode(new Quid::InProcessNode(*core, *protocol));
