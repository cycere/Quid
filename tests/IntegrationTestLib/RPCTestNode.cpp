...
bool RPCTestNode::getBlockTemplate(const std::string& minerAddress, Quid::Block& blockTemplate, uint64_t& difficulty) {
...
bool RPCTestNode::makeINode(std::unique_ptr<Quid::INode>& node) {
  std::unique_ptr<Quid::INode> newNode(new Quid::NodeRpcProxy("127.0.0.1", m_rpcPort));
  NodeCallback cb;
...
uint64_t RPCTestNode::getLocalHeight() {
  try {
    Quid::COMMAND_RPC_GET_INFO::request req;
    Quid::COMMAND_RPC_GET_INFO::response rsp;
