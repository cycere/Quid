...
namespace Tests {
  ...
  namespace po = boost::program_options;
    ...
    public:
      BaseFunctionalTests(const Quid::Currency& currency, System::Dispatcher& d, const BaseFunctionalTestsConfig& config) :
          m_dispatcher(d),
          m_currency(currency),
    ...
    protected:
      ...
      bool makeWallet(std::unique_ptr<Quid::IWalletLegacy> & wallet, std::unique_ptr<Quid::INode>& node, const std::string& password = "pass");
      bool mineBlocks(TestNode& node, const Quid::AccountPublicAddress& address, size_t blockCount);
      bool mineBlock(std::unique_ptr<Quid::IWalletLegacy>& wallet);
      bool mineBlock();
      bool startMining(size_t threads);
      bool stopMining();

      bool getNodeTransactionPool(size_t nodeIndex, Quid::INode& node, std::vector<std::unique_ptr<Quid::ITransactionReader>>& txPool);

      bool waitDaemonsReady();
      bool waitDaemonReady(size_t nodeIndex);
      bool waitForPeerCount(Quid::INode& node, size_t expectedPeerCount);
      bool waitForPoolSize(size_t nodeIndex, Quid::INode& node, size_t expectedPoolSize,
        std::vector<std::unique_ptr<Quid::ITransactionReader>>& txPool);

      bool prepareAndSubmitBlock(TestNode& node, Quid::Block&& blockTemplate);
    ...
    private:
      ...
      std::unique_ptr<Quid::INode> mainNode;
      std::unique_ptr<Quid::IWalletLegacy> workingWallet;
