...
#ifdef _WIN32
const std::string DAEMON_FILENAME = std::string(QUID_NAME) + "d.exe";
#else
const std::string DAEMON_FILENAME = std::string(QUID_NAME) + "d";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_" << QUID_NAME << "d_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"" + std::string(QUID_NAME) + "d" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), (std::string(QUID_NAME) + "d").c_str(), "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
...
namespace {
  class WaitForCoinBaseObserver : public Quid::IWalletLegacyObserver {
    Semaphore& m_gotReward;
    Quid::IWalletLegacy& m_wallet;
  public:
    WaitForCoinBaseObserver(Semaphore& gotReward, Quid::IWalletLegacy& wallet) : m_gotReward(gotReward), m_wallet(wallet) { }
    virtual void externalTransactionCreated(Quid::TransactionId transactionId) override {
      Quid::WalletLegacyTransaction trInfo;
...
bool BaseFunctionalTests::mineBlocks(TestNode& node, const Quid::AccountPublicAddress& address, size_t blockCount) {
...
bool BaseFunctionalTests::prepareAndSubmitBlock(TestNode& node, Quid::Block&& blockTemplate) {
  blockTemplate.timestamp = m_nextTimestamp;
  m_nextTimestamp += 2 * m_currency.difficultyTarget();

  BinaryArray blockBlob = Quid::toBinaryArray(blockTemplate);
  return node.submitBlock(::Common::toHex(blockBlob.data(), blockBlob.size()));
}

bool BaseFunctionalTests::mineBlock(std::unique_ptr<Quid::IWalletLegacy> &wallet) {
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<Quid::IWalletLegacy> & wallet, std::unique_ptr<Quid::INode>& node, const std::string& password) {
  if (!node) return false;
  wallet = std::unique_ptr<Quid::IWalletLegacy>(new Quid::WalletLegacy(m_currency, *node));
...
namespace {
  struct PeerCountWaiter : Quid::INodeObserver {
...
bool BaseFunctionalTests::waitForPeerCount(Quid::INode& node, size_t expectedPeerCount) {
...
bool BaseFunctionalTests::waitForPoolSize(size_t nodeIndex, Quid::INode& node, size_t expectedPoolSize,
  std::vector<std::unique_ptr<Quid::ITransactionReader>>& txPool) {
...
bool BaseFunctionalTests::getNodeTransactionPool(size_t nodeIndex, Quid::INode& node,
  std::vector<std::unique_ptr<Quid::ITransactionReader>>& txPool) {
