...
uint64_t test_generator::getAlreadyGeneratedCoins(const Quid::Block& blk) const {
  Crypto::Hash blkHash;
  get_block_hash(blk, blkHash);
  return getAlreadyGeneratedCoins(blkHash);
}

void test_generator::addBlock(const Quid::Block& blk, size_t tsxSize, uint64_t fee,
...
bool test_generator::constructBlock(Quid::Block& blk, uint32_t height, const Crypto::Hash& previousBlockHash,
...
bool test_generator::constructBlock(Quid::Block& blk, const Quid::AccountBase& minerAcc, uint64_t timestamp) {
  std::vector<size_t> blockSizes;
  std::list<Quid::Transaction> txList;
  return constructBlock(blk, 0, NULL_HASH, minerAcc, timestamp, 0, blockSizes, txList);
}

bool test_generator::constructBlock(Quid::Block& blk, const Quid::Block& blkPrev,
                                    const Quid::AccountBase& minerAcc,
...
bool test_generator::constructBlockManuallyTx(Quid::Block& blk, const Quid::Block& prevBlock,
                                              const Quid::AccountBase& minerAcc,
...
bool test_generator::constructMaxSizeBlock(Quid::Block& blk, const Quid::Block& blkPrev,
                                           const Quid::AccountBase& minerAccount,
...
void fillNonce(Quid::Block& blk, const difficulty_type& diffic) {
  blk.nonce = 0;
  Crypto::cn_context context;
...
bool constructMinerTxBySize(const Quid::Currency& currency, Quid::Transaction& baseTransaction, uint32_t height,
  ...
  if (!currency.constructMinerTx(height, Common::medianValue(blockSizes), alreadyGeneratedCoins, targetBlockSize,
      fee, minerAddress, baseTransaction, Quid::BinaryArray(), 1)) {
    return false;
  }
