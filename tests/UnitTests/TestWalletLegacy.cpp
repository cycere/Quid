...
class TrivialWalletObserver : public Quid::IWalletLegacyObserver
{
public:
  ...
  virtual void sendTransactionCompleted(Quid::TransactionId transactionId, std::error_code result) override {
    sendResult = result;
    sent.notify();
  }
...
struct SaveOnInitWalletObserver: public Quid::IWalletLegacyObserver {
  SaveOnInitWalletObserver(Quid::WalletLegacy* wallet) : wallet(wallet) {
  };
  ...
  Quid::WalletLegacy* wallet;
  std::stringstream stream;
};

static const uint64_t TEST_BLOCK_REWARD = 70368744177663;

Quid::TransactionId TransferMoney(Quid::WalletLegacy& from, Quid::WalletLegacy& to, int64_t amount, uint64_t fee, uint64_t mixIn = 0, const std::string& extra = "") {
  Quid::WalletLegacyTransfer transfer;
...
{
public:
  WalletLegacyApi() : m_currency(Quid::CurrencyBuilder(m_logger).currency()), generator(m_currency) {
  }
...
protected:
  ...
  void GetOneBlockReward(Quid::WalletLegacy& wallet);
  void GetOneBlockReward(Quid::WalletLegacy& wallet, const Quid::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
  void GetOneBlockRewardAndUnlock(Quid::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                  const Quid::Currency& currency, TestBlockchainGenerator& blockchainGenerator);
...
void WalletLegacyApi::prepareAliceWallet() {
  ...
  alice.reset(new Quid::WalletLegacy(m_currency, *newNode));
  aliceNode = newNode;
...
void WalletLegacyApi::prepareBobWallet() {
  ...
  bob.reset(new Quid::WalletLegacy(m_currency, *bobNode));
...
void WalletLegacyApi::prepareCarolWallet() {
  ...
  carol.reset(new Quid::WalletLegacy(m_currency, *carolNode));
  carol->addObserver(carolWalletObserver.get());
}

void WalletLegacyApi::GetOneBlockReward(Quid::WalletLegacy& wallet) {
  GetOneBlockReward(wallet, m_currency, generator);
}

void WalletLegacyApi::GetOneBlockReward(Quid::WalletLegacy& wallet, const Quid::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::GetOneBlockRewardAndUnlock(Quid::WalletLegacy& wallet, TrivialWalletObserver& observer, INodeTrivialRefreshStub& node,
                                                 const Quid::Currency& currency, TestBlockchainGenerator& blockchainGenerator) {
...
void WalletLegacyApi::performTransferWithErrorTx(const std::array<int64_t, 5>& amounts, uint64_t fee) {
  std::vector<Quid::WalletLegacyTransfer> trs;
  Quid::WalletLegacyTransfer tr;
...
TEST_F(WalletLegacyApi, TransactionsAndTransfersAfterSend) {
  ...
  Quid::WalletLegacyTransaction tx;
...
TEST_F(WalletLegacyApi, getTransactionSuccess) {
  ...
  EXPECT_EQ(tx.firstTransferId, Quid::WALLET_LEGACY_INVALID_TRANSFER_ID);
...
TEST_F(WalletLegacyApi, wrongPassword) {
  ...
  EXPECT_EQ(result.value(), Quid::error::WRONG_PASSWORD);
}
...
TEST_F(WalletLegacyApi, sendMoneyToMyself) {
  ...
  Quid::TransactionId txId = TransferMoney(*alice, *alice, 100000000, 100);
  ASSERT_NE(txId, Quid::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, checkBalanceAfterSend) {
  ...
  Quid::TransactionId txId = TransferMoney(*alice, *alice, sendAmount, fee);
  ASSERT_NE(txId, Quid::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
...
TEST_F(WalletLegacyApi, moneyInPoolDontAffectActualBalance) {
  ...
  Quid::TransactionId txId = TransferMoney(*alice, *bob, sendAmount, fee);
  ASSERT_NE(txId, Quid::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  generator.generateEmptyBlocks(10);
...
TEST_F(WalletLegacyApi, sendAfterFailedTransaction) {
  ...
  Quid::TransactionId txId = TransferMoney(*alice, *alice, 100000, 100);
  ASSERT_NE(txId, Quid::WALLET_LEGACY_INVALID_TRANSACTION_ID);
  ASSERT_NO_FATAL_FAILURE(WaitWalletSend(aliceWalletObserver.get()));
  alice->shutdown();
}
...
TEST_F(WalletLegacyApi, outcommingExternalTransactionTotalAmount) {
  class ExternalTxChecker : public Quid::IWalletLegacyObserver {
  public:
    ExternalTxChecker(Quid::WalletLegacy& wallet) : wallet(wallet), totalAmount(std::numeric_limits<int64_t>::max()) {
    }

    virtual void externalTransactionCreated(Quid::TransactionId transactionId) override {
      Quid::WalletLegacyTransaction txInfo;
  ...
  Quid::WalletLegacy wallet(m_currency, *aliceNode);
...
namespace {
class WalletSynchronizationProgressUpdatedObserver : public Quid::IWalletLegacyObserver {
public:
...
TEST_F(WalletLegacyApi, shutdownDoesNotRemoveObservers) {
  ...
  Quid::WalletHelper::IWalletRemoveObserverGuard observerGuard(*alice, observer);
...
namespace {
class WalletTransactionEventCounter : public Quid::IWalletLegacyObserver {
public:
  virtual void externalTransactionCreated(Quid::TransactionId /*transactionId*/) override {
    ++m_count;
  }

  virtual void transactionUpdated(Quid::TransactionId /*transactionId*/) override {
    ++m_count;
  }
...
TEST_F(WalletLegacyApi, afterShutdownAndInitWalletDoesNotSendNotificationsRelatedToOldAddress) {
  ...
  std::vector<Quid::WalletLegacyTransfer> transfers;
...
TEST_F(WalletLegacyApi, resetDoesNotChangeAccountKeys) {
  ...
  Quid::AccountKeys expectedAccountKeys;
  alice->getAccountKeys(expectedAccountKeys);

  alice->reset();

  Quid::AccountKeys actualAccountKeys;
...
void generateWallet(Quid::IWalletLegacy& wallet, TrivialWalletObserver& observer, const std::string& pass) {
...
TEST_F(WalletLegacyApi, outdatedUnconfirmedTransactionDeletedOnNewBlock) {
  ...
  Quid::Currency currency(Quid::CurrencyBuilder(m_logger).mempoolTxLiveTime(TRANSACTION_MEMPOOL_TIME).currency());
  TestBlockchainGenerator blockchainGenerator(currency);
  INodeTrivialRefreshStub node(blockchainGenerator);
  Quid::WalletLegacy wallet(currency, node);
  TrivialWalletObserver walletObserver;
  wallet.addObserver(&walletObserver);
  ...
  Quid::WalletLegacyTransaction transaction;
  ASSERT_TRUE(wallet.getTransaction(id, transaction));
  EXPECT_EQ(Quid::WalletLegacyTransactionState::Deleted, transaction.state);
...
TEST_F(WalletLegacyApi, walletLoadsNullSpendSecretKey) {
  ...
  Crypto::generate_keys(accountKeys.address.spendPublicKey, accountKeys.spendSecretKey);
  Crypto::generate_keys(accountKeys.address.viewPublicKey, accountKeys.viewSecretKey);
  accountKeys.spendSecretKey = Quid::NULL_SECRET_KEY;
