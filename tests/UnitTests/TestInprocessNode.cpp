...
namespace {
Quid::Transaction createTx(Quid::ITransactionReader& tx) {
  Quid::Transaction outTx;
...
protected:
  ...
  ICoreStub coreStub;
  IQuidProtocolQueryStub protocolQueryStub;
  Quid::InProcessNode node;
...
TEST_F(InProcessNodeTests, initOk) {
  Quid::InProcessNode newNode(coreStub, protocolQueryStub);
  CallbackStatus status;
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  ...
  Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response expectedResp;
  Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount out;
  out.amount = 10;
  out.outs.push_back({ 11, ignoredPublicKey });
  expectedResp.outs.push_back(out);
  coreStub.set_random_outs(expectedResp, true);

  std::vector<Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_outs_for_amount> outs;
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized) {
  ...
  std::vector<Quid::block_complete_entry> newBlocks;
...
TEST_F(InProcessNodeTests, relayTransactionUninitialized) {
  ...
  CallbackStatus status;
  newNode.relayTransaction(Quid::Transaction(), [&] (std::error_code ec) { status.setStatus(ec); });
  ASSERT_TRUE(status.wait());
  ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightEmpty) {
  ...
  std::vector<std::vector<Quid::BlockDetails>> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  std::vector<uint32_t> blockHeights;
  std::vector<std::vector<Quid::BlockDetails>> actualBlocks;

  std::vector<Quid::Block> expectedBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    ...
    blockHeights.push_back(std::move(boost::get<Quid::BaseInput>(iter->baseTransaction.inputs.front()).blockIndex));
  ...
  for (const boost::tuple<boost::tuple<size_t, Quid::Block>, std::vector<Quid::BlockDetails>>& sameHeight : range) {
    EXPECT_EQ(sameHeight.get<1>().size(), 1);
    for (const Quid::BlockDetails& block : sameHeight.get<1>()) {
      EXPECT_EQ(block.height, sameHeight.get<0>().get<0>());
      Crypto::Hash expectedCryptoHash = Quid::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHeightFail) {
  ...
  for (const Quid::Block& block : generator.getBlockchain()) {
...
TEST_F(InProcessNodeTests, getBlocksByHashEmpty) {
  ...
  std::vector<Quid::BlockDetails> blocks;
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  std::vector<Crypto::Hash> blockHashes;
  std::vector<Quid::BlockDetails> actualBlocks;
  ...
  for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++) {
    expectedBlocks.push_back(*iter);
    blockHashes.push_back(Quid::get_block_hash(*iter));
    coreStub.addBlock(*iter);
  }
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<Crypto::Hash, Quid::Block>, Quid::BlockDetails>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = Quid::get_block_hash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxEmpty) {
  ...
  std::vector<Quid::TransactionDetails> transactions;
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  std::vector<Crypto::Hash> transactionHashes;
  std::vector<Quid::TransactionDetails> actualTransactions;

  std::vector<std::tuple<Quid::Transaction, Crypto::Hash, uint64_t>> expectedTransactions;

  coreStub.set_blockchain_top(0, boost::value_initialized<Crypto::Hash>());

  size_t prevBlockchainSize = generator.getBlockchain().size();
  for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i) {
    auto txptr = Quid::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.push_back(Quid::getObjectHash(tx));
    ...
    expectedTransactions.push_back(std::make_tuple(tx, Quid::get_block_hash(generator.getBlockchain().back()), boost::get<Quid::BaseInput>(generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
  }
  ...
  for (const boost::tuple<boost::tuple<Crypto::Hash, Quid::TransactionDetails>, std::tuple<Quid::Transaction, Crypto::Hash, uint64_t>>& sameHeight : range) {
    Crypto::Hash expectedCryptoHash = Quid::getObjectHash(std::get<0>(sameHeight.get<1>()));
    EXPECT_EQ(expectedCryptoHash, sameHeight.get<0>().get<0>());
    Hash expectedHash = reinterpret_cast<const Hash&>(expectedCryptoHash);
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp) {
  ...
  Quid::InProcessNode newNode(core, protocolQueryStub);
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    auto txptr = Quid::createTransaction();
    auto tx = ::createTx(*txptr.get());
    transactionHashes.insert(Quid::getObjectHash(tx));
    Quid::tx_verification_context tvc = boost::value_initialized<tx_verification_context>();
    bool keptByBlock = false;
    coreStub.handleIncomingTransaction(tx, Quid::getObjectHash(tx), Quid::getObjectBinarySize(tx), tvc, keptByBlock);
  ...
  std::vector<Crypto::Hash> knownPoolTxIds;
  Crypto::Hash knownBlockId = Quid::getObjectHash(generator.getBlockchain().back());
  bool isBcActual = false;
  std::vector<std::unique_ptr<ITransactionReader>> newTxs;
  std::vector<Crypto::Hash> deletedTxIds;
