...
#include "QuidCore/TransactionPool.h"
...
class TransactionValidator : public Quid::ITransactionValidator {
  virtual bool checkTransactionInputs(const Quid::Transaction& tx, BlockInfo& maxUsedBlock) override {
    return true;
  }

  virtual bool checkTransactionInputs(const Quid::Transaction& tx, BlockInfo& maxUsedBlock, BlockInfo& lastFailed) override {
    return true;
  }

  virtual bool haveSpentKeyImages(const Quid::Transaction& tx) override {
    return false;
  }
...
public:
  ...
  TestTransactionGenerator(const Quid::Currency& currency, size_t ringSize) :
    m_currency(currency),
    m_ringSize(ringSize),
    m_miners(ringSize), 
...
namespace
{
  ...
  void GenerateTransaction(const Quid::Currency& currency, Transaction& tx, uint64_t fee, size_t outputs) {
  ...
  public:
    ...
    TestPool(const Quid::Currency& currency, Logging::ILogger& logger) :
...
public:
  TxPool_FillBlockTemplate() :
    tx_pool() {
    currency = Quid::CurrencyBuilder(logger).fusionTxMaxSize(TEST_FUSION_TX_MAX_SIZE).blockGrantedFullRewardZone(TEST_MEDIAN_SIZE).currency();
  }
