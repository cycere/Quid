...
ICoreStub::ICoreStub(const Quid::Block& genesisBlock) :
    topHeight(0),
    globalIndicesResult(false),
    randomOutsResult(false),
    poolTxVerificationResult(true),
...
bool ICoreStub::addObserver(Quid::ICoreObserver* observer) {
  return true;
}

bool ICoreStub::removeObserver(Quid::ICoreObserver* observer) {
  return true;
}
...
bool ICoreStub::get_random_outs_for_amounts(const Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request& req,
    Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& res) {
  res = randomOuts;
...
Quid::i_cryptonote_protocol* ICoreStub::get_protocol() {
  return nullptr;
}

bool ICoreStub::handle_incoming_tx(Quid::BinaryArray const& tx_blob, Quid::tx_verification_context& tvc, bool keeped_by_block) {
  return true;
}
...
void ICoreStub::set_random_outs(const Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp, bool result) {
  randomOuts = resp;
  randomOutsResult = result;
}

std::vector<Quid::Transaction> ICoreStub::getPoolTransactions() {
  return std::vector<Quid::Transaction>();
}

bool ICoreStub::getPoolChanges(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
                               std::vector<Quid::Transaction>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  ...
  for (const std::pair<Crypto::Hash, Quid::Transaction>& poolEntry : transactionPool) {
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash& tailBlockId, const std::vector<Crypto::Hash>& knownTxsIds,
        std::vector<Quid::TransactionPrefixInfo>& addedTxs, std::vector<Crypto::Hash>& deletedTxsIds) {
  std::vector<Quid::Transaction> added;
  bool returnStatus = getPoolChanges(tailBlockId, knownTxsIds, added, deletedTxsIds);

  for (const auto& tx : added) {
    Quid::TransactionPrefixInfo tpi;
    tpi.txPrefix = tx;
...
void ICoreStub::getPoolChanges(const std::vector<Crypto::Hash>& knownTxsIds, std::vector<Quid::Transaction>& addedTxs,
                               std::vector<Crypto::Hash>& deletedTxsIds) {
}

bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<Quid::BlockFullInfo>& entries) {
  //stub
  return true;
}

bool ICoreStub::queryBlocksLite(const std::vector<Crypto::Hash>& block_ids, uint64_t timestamp,
  uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<Quid::BlockShortInfo>& entries) {
  //stub
  return true;
}
...
size_t ICoreStub::addChain(const std::vector<const Quid::IBlock*>& chain) {
  size_t blocksCounter = 0;
  for (const Quid::IBlock* block : chain) {
    for (size_t txNumber = 0; txNumber < block->getTransactionCount(); ++txNumber) {
      const Quid::Transaction& tx = block->getTransaction(txNumber);
      Crypto::Hash txHash = Quid::NULL_HASH;
      size_t blobSize = 0;
...
Crypto::Hash ICoreStub::getBlockIdByHeight(uint32_t height) {
  ...
  if (iter == blockHashByHeightIndex.end()) {
    return Quid::NULL_HASH;
  }
  return iter->second;
}

bool ICoreStub::getBlockByHash(const Crypto::Hash &h, Quid::Block &blk) {
...
void ICoreStub::getTransactions(const std::vector<Crypto::Hash>& txs_ids, std::list<Quid::Transaction>& txs, std::list<Crypto::Hash>& missed_txs, bool checkTxPool) {
...
bool ICoreStub::scanOutputkeysForIndices(const Quid::KeyInput& txInToKey, std::list<std::pair<Crypto::Hash, size_t>>& outputReferences) {
  return true;
}

bool ICoreStub::getBlockDifficulty(uint32_t height, Quid::difficulty_type& difficulty) {
  return true;
}
...
bool ICoreStub::getBlockContainingTx(const Crypto::Hash& txId, Crypto::Hash& blockId, uint32_t& blockHeight) {
  ...
  blockHeight = boost::get<Quid::BaseInput>(blockIter->second.baseTransaction.inputs.front()).blockIndex;
  return true;
}

bool ICoreStub::getMultisigOutputReference(const Quid::MultisignatureInput& txInMultisig, std::pair<Crypto::Hash, size_t>& outputReference) {
  return true;
}

void ICoreStub::addBlock(const Quid::Block& block) {
  uint32_t height = boost::get<Quid::BaseInput>(block.baseTransaction.inputs.front()).blockIndex;
  Crypto::Hash hash = Quid::get_block_hash(block);
  if (height > topHeight) {
    topHeight = height;
    topId = hash;
  }
  ...
  blockHashByTxHashIndex.emplace(std::make_pair(Quid::getObjectHash(block.baseTransaction), hash));
  for (auto txHash : block.transactionHashes) {
    blockHashByTxHashIndex.emplace(std::make_pair(txHash, hash));
  }
}

void ICoreStub::addTransaction(const Quid::Transaction& tx) {
  Crypto::Hash hash = Quid::getObjectHash(tx);
...
bool ICoreStub::getOrphanBlocksByHeight(uint32_t height, std::vector<Quid::Block>& blocks) {
  return true;
}

bool ICoreStub::getBlocksByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<Quid::Block>& blocks, uint32_t& blocksNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getPoolTransactionsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Quid::Transaction>& transactions, uint64_t& transactionsNumberWithinTimestamps) {
  return true;
}

bool ICoreStub::getTransactionsByPaymentId(const Crypto::Hash& paymentId, std::vector<Quid::Transaction>& transactions) {
  return true;
}
...
std::unique_ptr<Quid::IBlock> ICoreStub::getBlock(const Crypto::Hash& blockId) {
  return std::unique_ptr<Quid::IBlock>(nullptr);
}

bool ICoreStub::handleIncomingTransaction(const Quid::Transaction& tx, const Crypto::Hash& txHash, size_t blobSize, Quid::tx_verification_context& tvc, bool keptByBlock) {
...
bool ICoreStub::addMessageQueue(Quid::MessageQueue<Quid::BlockchainMessage>& messageQueuePtr) {
  return true;
}

bool ICoreStub::removeMessageQueue(Quid::MessageQueue<Quid::BlockchainMessage>& messageQueuePtr) {
  return true;
}
