...
TEST(KVSerialize, Simple) {
  ...
  std::string buf = Quid::storeToBinaryKeyValue(testData1);
  ASSERT_TRUE(Quid::loadFromBinaryKeyValue(testData2, buf));
...
TEST(KVSerialize, BigCollection) {
  ...
  std::string buf = Quid::storeToBinaryKeyValue(ts1);
  ASSERT_TRUE(Quid::loadFromBinaryKeyValue(ts2, buf));
  EXPECT_EQ(ts1, ts2);
}
