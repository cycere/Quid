...
{
  std::string buff;
  Quid::NOTIFY_RESPONSE_CHAIN_ENTRY::request r;
  r.start_height = 1;
  r.total_height = 3;
  for(int i = 1; i < 10000; i += i*10) {
    r.m_block_ids.resize(i, Quid::NULL_HASH);
    buff = Quid::storeToBinaryKeyValue(r);

    Quid::NOTIFY_RESPONSE_CHAIN_ENTRY::request r2;
    Quid::loadFromBinaryKeyValue(r2, buff);
