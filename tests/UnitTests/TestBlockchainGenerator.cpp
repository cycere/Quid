...
public:
  ...
  void generate(const AccountPublicAddress& address, Transaction& tx, uint64_t unlockTime = 0)
  {
    ...
    Quid::decompose_amount_into_digits(this->m_source_amount, 0,
      [&](uint64_t chunk) { destinations.push_back(Quid::TransactionDestinationEntry(chunk, address)); },
      [&](uint64_t a_dust) { destinations.push_back(Quid::TransactionDestinationEntry(a_dust, address)); });

    Quid::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, destinations, std::vector<uint8_t>(), tx, unlockTime, m_logger);
  }
...
TestBlockchainGenerator::TestBlockchainGenerator(const Quid::Currency& currency) :
  m_currency(currency),
  generator(currency)
{
...
std::vector<Quid::Block>& TestBlockchainGenerator::getBlockchain()
{
  std::unique_lock<std::mutex> lock(m_mutex);
  return m_blockchain;
}

std::vector<Quid::Block> TestBlockchainGenerator::getBlockchainCopy() {
  std::unique_lock<std::mutex> lock(m_mutex);

  std::vector<Quid::Block> blockchain(m_blockchain);
  return blockchain;
}

bool TestBlockchainGenerator::getTransactionByHash(const Crypto::Hash& hash, Quid::Transaction& tx, bool checkTxPool)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
const Quid::AccountBase& TestBlockchainGenerator::getMinerAccount() const {
  std::unique_lock<std::mutex> lock(m_mutex);
  return miner_acc;
}
...
void TestBlockchainGenerator::addGenesisBlock() {
  ...
  m_timestampIndex.add(m_currency.genesisBlock().timestamp, Quid::get_block_hash(m_currency.genesisBlock()));
...
void TestBlockchainGenerator::addMiningBlock() {
  ...
  Quid::Block& prev_block = m_blockchain.back();
  ...
  m_timestampIndex.add(block.timestamp, Quid::get_block_hash(block));
...
void TestBlockchainGenerator::addTxToBlockchain(const Quid::Transaction& transaction)
{
  std::unique_lock<std::mutex> lock(m_mutex);
  addToBlockchain(transaction);
}

bool TestBlockchainGenerator::getBlockRewardForAddress(const Quid::AccountPublicAddress& address)
{
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::generateTransactionsInOneBlock(const Quid::AccountPublicAddress& address, size_t n) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
bool TestBlockchainGenerator::getSingleOutputTransaction(const Quid::AccountPublicAddress& address, uint64_t amount) {
  std::unique_lock<std::mutex> lock(m_mutex);
...
void TestBlockchainGenerator::addToBlockchain(const Quid::Transaction& tx) {
  addToBlockchain(std::vector<Quid::Transaction> {tx});
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<Quid::Transaction>& txs) {
  addToBlockchain(txs, miner_acc);
}

void TestBlockchainGenerator::addToBlockchain(const std::vector<Quid::Transaction>& txs, const Quid::AccountBase& minerAddress) {
  std::list<Quid::Transaction> txsToBlock;
  ...
  std::vector<Quid::Transaction>& new_txs, std::vector<Crypto::Hash>& deleted_tx_ids)
{
  ...
  if (known_block_id != Quid::get_block_hash(m_blockchain.back())) {
    is_bc_actual = false;
    return;
  }
...
void TestBlockchainGenerator::putTxToPool(const Quid::Transaction& tx) {
  std::unique_lock<std::mutex> lock(m_mutex);

  Crypto::Hash txHash = Quid::getObjectHash(tx);
  m_txPool[txHash] = tx;
}

void TestBlockchainGenerator::putTxPoolToBlockchain() {
  std::unique_lock<std::mutex> lock(m_mutex);
  std::vector<Quid::Transaction> txs;
...
void TestBlockchainGenerator::setMinerAccount(const Quid::AccountBase& account) {
  miner_acc = account;
}
...
bool TestBlockchainGenerator::getPoolTransactionIdsByTimestamp(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t transactionsNumberLimit, std::vector<Crypto::Hash>& hashes, uint64_t& transactionsNumberWithinTimestamps) {
  ...
  for (auto i : m_txPool) {
    ...
    hashes.push_back(Quid::getObjectHash(i.second));
    ++c;
  }
  return true;
}
...
void TestBlockchainGenerator::addTx(const Quid::Transaction& tx) {
...
bool TestBlockchainGenerator::generateFromBaseTx(const Quid::AccountBase& address) {
  std::unique_lock<std::mutex> lock(m_mutex);
