...
{
  Quid::AccountPublicAddress addr;

  ASSERT_NO_THROW(Quid::loadFromBinary(addr, Common::asBinaryArray(test_serialized_keys)));
  std::string addr_str = Quid::getAccountAddressAsStr(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, addr);
...
TEST(parseAccountAddressString, handles_valid_address)
{
  uint64_t prefix;
  Quid::AccountPublicAddress addr;
  ASSERT_TRUE(Quid::parseAccountAddressString(prefix, addr, test_keys_addr_str));
  ASSERT_EQ(TEST_PUBLIC_ADDRESS_BASE58_PREFIX, prefix);

  BinaryArray blob;
  ASSERT_NO_THROW(blob = Quid::storeToBinary(addr));
...
TEST(parseAccountAddressString, fails_on_invalid_address_format)
{
  ...
  uint64_t prefix;
  Quid::AccountPublicAddress addr;
  ASSERT_FALSE(Quid::parseAccountAddressString(prefix, addr, addr_str));
}
