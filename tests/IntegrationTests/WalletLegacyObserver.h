...
public:
  ...
  Quid::TransactionId waitExternalTransaction() {
    std::unique_lock<std::mutex> lk(m_mutex);
    ...
    Quid::TransactionId txId = m_externalTransactions.front();
  ...
  std::error_code waitSendResult(Quid::TransactionId txid) {
    std::unique_lock<std::mutex> lk(m_mutex);

    std::unordered_map<Quid::TransactionId, std::error_code>::iterator it;
...
private:
  ...
  std::unordered_map<Quid::TransactionId, std::error_code> m_sendResults;
  std::deque<Quid::TransactionId> m_externalTransactions;
};
