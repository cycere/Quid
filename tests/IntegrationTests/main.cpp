...
public:
  ...
  SimpleTest(const Quid::Currency& currency, System::Dispatcher& system, const Tests::Common::BaseFunctionalTestsConfig& config) : 
    BaseFunctionalTests(currency, system, config) {}

  class WaitForActualGrowObserver : public Quid::IWalletLegacyObserver {
  ...
  class WaitForActualDwindleObserver : public Quid::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForPendingGrowObserver : public Quid::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_GotActual;
  ...
  class WaitForConfirmationObserver : public Quid::IWalletLegacyObserver {
  ...
  class WaitForSendCompletedObserver : public Quid::IWalletLegacyObserver {
    Tests::Common::Semaphore& m_Sent;
    std::error_code& m_error;
    Quid::TransactionId& m_transactionId;

  public:
    WaitForSendCompletedObserver(Tests::Common::Semaphore& Sent, Quid::TransactionId& transactionId, std::error_code& error) : m_Sent(Sent), m_transactionId(transactionId), m_error(error) { }
  ...
  class WaitForExternalTransactionObserver : public Quid::IWalletLegacyObserver {
  public:
    WaitForExternalTransactionObserver() { }
    std::promise<Quid::TransactionId> promise;
  ...
  class WaitForTransactionUpdated : public Quid::IWalletLegacyObserver {
  public:
    ...
    Quid::TransactionId expectindTxId;
  };
  ...
  bool perform1() {
    ...
    std::unique_ptr<Quid::INode> node1;
    std::unique_ptr<Quid::INode> node2;

    nodeDaemons.front()->makeINode(node1);
    nodeDaemons.front()->makeINode(node2);

    std::unique_ptr<Quid::IWalletLegacy> wallet1;
    std::unique_ptr<Quid::IWalletLegacy> wallet2;
  ...
  class WaitForBlockchainHeightChangeObserver : public Quid::INodeObserver {
  ...
  class CallbackHeightChangeObserver : public Quid::INodeObserver {
  ...
  bool perform2(size_t blocksCount = 10)
  {
    ...
    mineBlock();
    LOG_TRACE("STEP 2 PASSED");
    std::unique_ptr<Quid::INode> localNode;
    std::unique_ptr<Quid::INode> remoteNode;

    nodeDaemons.front()->makeINode(localNode);
    nodeDaemons.back()->makeINode(remoteNode);
    
    std::unique_ptr<Quid::IWalletLegacy> wallet;
  ...
  bool perform4() {
    ...
    std::unique_ptr<Quid::INode> hopNode;
    std::unique_ptr<Quid::INode> localNode;
    std::unique_ptr<Quid::INode> remoteNode;
  ...
  bool perform5() {
    ...
    std::unique_ptr<Quid::INode> node1;
    std::unique_ptr<Quid::INode> inprocNode;
...
void testMultiVersion(const Quid::Currency& currency, System::Dispatcher& d, const Tests::Common::BaseFunctionalTestsConfig& config);
...
public:
  ...
  SimpleTestCase() : 
    currency(Quid::CurrencyBuilder(logger).testnet(true).currency()), 
...
TEST_F(SimpleTestCase, TESTPOOLDELETION) {
  currency = Quid::CurrencyBuilder(logger).testnet(true).mempoolTxLiveTime(60).currency();
