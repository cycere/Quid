...
public:
  ...
  IntegrationTest() : 
    currency(Quid::CurrencyBuilder(log).testnet(true).currency()), 
  ...
  void makeWallets() {
    for (auto& n: inodes) {
      std::unique_ptr<Quid::IWalletLegacy> wallet(new Quid::WalletLegacy(m_currency, *n));
...
TEST_F(IntegrationTest, BlockPropagationSpeed) {
  ...
  {
    std::unique_ptr<Quid::INode>& localNode = inodes.front();
    std::unique_ptr<Quid::INode>& remoteNode = inodes.back();
