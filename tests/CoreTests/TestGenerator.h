...
public:
  ...
  const Quid::Currency& currency() const { return generator.currency(); }

  void makeNextBlock(const std::list<Quid::Transaction>& txs = std::list<Quid::Transaction>()) {
    Quid::Block block;
  ...
  void makeNextBlock(const Quid::Transaction& tx) {
    std::list<Quid::Transaction> txs;
    txs.push_back(tx);
    makeNextBlock(txs);
  }
  ...
  void generateBlocks(size_t count, uint8_t majorVersion = Quid::BLOCK_MAJOR_VERSION_1) {
    while (count--) {
      Quid::Block next;
  ...
  TransactionBuilder createTxBuilder(const Quid::AccountBase& from, const Quid::AccountBase& to, uint64_t amount, uint64_t fee) {
  ...
  void fillTxSourcesAndDestinations(
    std::vector<Quid::TransactionSourceEntry>& sources, 
    std::vector<Quid::TransactionDestinationEntry>& destinations,
    const Quid::AccountBase& from, const Quid::AccountBase& to, uint64_t amount, uint64_t fee, size_t nmix = 0) {
    fill_tx_sources_and_destinations(events, lastBlock, from, to, amount, fee, nmix, sources, destinations);
  }

  void constructTxToKey(
    Quid::Transaction& tx,
    const Quid::AccountBase& from,
    const Quid::AccountBase& to,
    uint64_t amount,
    uint64_t fee,
    size_t nmix = 0) {
  ...
  Quid::Block genesisBlock;
  Quid::Block lastBlock;
  Quid::AccountBase minerAccount;
