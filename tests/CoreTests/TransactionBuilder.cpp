...
TransactionBuilder::TransactionBuilder(const Quid::Currency& currency, uint64_t unlockTime)
  : m_currency(currency), m_version(Quid::CURRENT_TRANSACTION_VERSION), m_unlockTime(unlockTime), m_txKey(generateKeyPair()) {}
...
TransactionBuilder& TransactionBuilder::setTxKeys(const Quid::KeyPair& txKeys) {
  m_txKey = txKeys;
  return *this;
}

TransactionBuilder& TransactionBuilder::setInput(const std::vector<Quid::TransactionSourceEntry>& sources, const Quid::AccountKeys& senderKeys) {
  m_sources = sources;
...
TransactionBuilder& TransactionBuilder::setOutput(const std::vector<Quid::TransactionDestinationEntry>& destinations) {
  m_destinations = destinations;
  return *this;
}

TransactionBuilder& TransactionBuilder::addOutput(const Quid::TransactionDestinationEntry& dest) {
...
Transaction TransactionBuilder::build() const {
  ...
  std::vector<Quid::KeyPair> contexts;
...
void TransactionBuilder::fillInputs(Transaction& tx, std::vector<Quid::KeyPair>& contexts) const {
...
void TransactionBuilder::signSources(const Crypto::Hash& prefixHash, const std::vector<Quid::KeyPair>& contexts, Transaction& tx) const {
