...
bool GetRandomOutputs::request(Quid::core& c, uint64_t amount, size_t mixin, Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response& resp) {
  Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_request req;

  req.amounts.push_back(amount);
  req.outs_count = mixin;

  resp = boost::value_initialized<Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response>();
...
bool GetRandomOutputs::checkHalfUnlocked(Quid::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
...
bool GetRandomOutputs::checkFullyUnlocked(Quid::core& c, size_t ev_index, const std::vector<test_event_entry>& events) {
  Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response resp;
