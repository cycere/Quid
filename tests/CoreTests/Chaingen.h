...
#include "QuidCore/TransactionExtra.h"
...
struct serialized_object
{
  ...
  serialized_object(const Quid::BinaryArray& a_data)
    : data(a_data)
  {
  }

  Quid::BinaryArray data;
  //BEGIN_SERIALIZE_OBJECT()
  //  FIELD(data)
  //  END_SERIALIZE()
...
typedef serialized_object<Quid::Block> serialized_block;
typedef serialized_object<Quid::Transaction> serialized_transaction;
...
//VARIANT_TAG(binary_archive, Quid::account_base, 0xcc);
//VARIANT_TAG(binary_archive, serialized_block, 0xcd);
//VARIANT_TAG(binary_archive, serialized_transaction, 0xce);
//VARIANT_TAG(binary_archive, event_visitor_settings, 0xcf);

typedef boost::variant<Quid::Block, Quid::Transaction, Quid::AccountBase, callback_entry, serialized_block, serialized_transaction, event_visitor_settings> test_event_entry;
typedef std::unordered_map<Crypto::Hash, const Quid::Transaction*> map_hash2tx_t;

class test_chain_unit_base: boost::noncopyable
{
public:
  test_chain_unit_base() :
    m_currency(Quid::CurrencyBuilder(m_logger).currency()) {
  }

  typedef std::function<bool (Quid::core& c, size_t ev_index, const std::vector<test_event_entry> &events)> verify_callback;
  typedef std::map<std::string, verify_callback> callbacks_map;

  const Quid::Currency& currency() const;
  void register_callback(const std::string& cb_name, verify_callback cb);
  bool verify(const std::string& cb_name, Quid::core& c, size_t ev_index, const std::vector<test_event_entry> &events);

protected:

  mutable Logging::ConsoleLogger m_logger;
  Quid::Currency m_currency;
...
bool construct_tx_to_key(Logging::ILogger& logger, const std::vector<test_event_entry>& events, Quid::Transaction& tx,
                         const Quid::Block& blk_head, const Quid::AccountBase& from, const Quid::AccountBase& to,
                         uint64_t amount, uint64_t fee, size_t nmix);
Quid::Transaction construct_tx_with_fee(Logging::ILogger& logger, std::vector<test_event_entry>& events, const Quid::Block& blk_head,
                                            const Quid::AccountBase& acc_from, const Quid::AccountBase& acc_to,
                                            uint64_t amount, uint64_t fee);

void get_confirmed_txs(const std::vector<Quid::Block>& blockchain, const map_hash2tx_t& mtx, map_hash2tx_t& confirmed_txs);
bool find_block_chain(const std::vector<test_event_entry>& events, std::vector<Quid::Block>& blockchain, map_hash2tx_t& mtx, const Crypto::Hash& head);
void fill_tx_sources_and_destinations(const std::vector<test_event_entry>& events, const Quid::Block& blk_head,
                                      const Quid::AccountBase& from, const Quid::AccountBase& to,
                                      uint64_t amount, uint64_t fee, size_t nmix,
                                      std::vector<Quid::TransactionSourceEntry>& sources,
                                      std::vector<Quid::TransactionDestinationEntry>& destinations);
uint64_t get_balance(const Quid::AccountBase& addr, const std::vector<Quid::Block>& blockchain, const map_hash2tx_t& mtx);

//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_tx_verification_context(const Quid::tx_verification_context& tvc, bool tx_added, size_t event_index, const Quid::Transaction& tx, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_tx_verification_context(const Quid::tx_verification_context& tvc, bool tx_added, size_t /*event_index*/, const Quid::Transaction& /*tx*/, t_test_class&, long)
{
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_tx_verification_context(const Quid::tx_verification_context& tvc, bool tx_added, size_t event_index, const Quid::Transaction& tx, t_test_class& validator)
{
  // SFINAE in action
...
//--------------------------------------------------------------------------
template<class t_test_class>
auto do_check_block_verification_context(const Quid::block_verification_context& bvc, size_t event_index, const Quid::Block& blk, t_test_class& validator, int)
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool do_check_block_verification_context(const Quid::block_verification_context& bvc, size_t /*event_index*/, const Quid::Block& /*blk*/, t_test_class&, long)
{
  // Default block verification context check
...
//--------------------------------------------------------------------------
template<class t_test_class>
bool check_block_verification_context(const Quid::block_verification_context& bvc, size_t event_index, const Quid::Block& blk, t_test_class& validator)
{
  // SFINAE in action
...
public:
  push_core_event_visitor(Quid::core& c, const std::vector<test_event_entry>& events, t_test_class& validator)
    : m_c(c)
    , m_events(events)
  ...
  bool operator()(const Quid::Transaction& tx) const
  {
    log_event("Quid::Transaction");
  ...
  bool operator()(const Quid::Block& b) const
  {
    log_event("Quid::Block");

    Quid::block_verification_context bvc = boost::value_initialized<decltype(bvc)>();
  ...
  bool operator()(const Quid::AccountBase& ab) const
  {
    log_event("Quid::account_base");
    return true;
  }
  ...
  bool operator()(const serialized_block& sr_block) const
  {
    ...
    Quid::Block blk;
    if (!Quid::fromBinaryArray(blk, sr_block.data)) {
      blk = Quid::Block();
    }
  ...
  bool operator()(const serialized_transaction& sr_tx) const
  {
    ...
    if (!Quid::fromBinaryArray(tx, sr_tx.data)) {
      tx = Quid::Transaction();
    }
...
//--------------------------------------------------------------------------
template<class t_test_class>
inline bool replay_events_through_core(Quid::core& cr, const std::vector<test_event_entry>& events, t_test_class& validator)
{
  try {
    CHECK_AND_ASSERT_MES(typeid(Quid::Block) == events[0].type(), false, "First event must be genesis block creation");
    cr.set_genesis_block(boost::get<Quid::Block>(events[0]));
...
inline bool do_replay_events(std::vector<test_event_entry>& events, t_test_class& validator)
{
  ...
  Quid::CoreConfig::initOptions(desc);
  ...
  Quid::CoreConfig coreConfig;
  coreConfig.init(vm);
  Quid::MinerConfig emptyMinerConfig;
  Quid::cryptonote_protocol_stub pr; //TODO: stub only for this kind of test, make real validation of relayed objects
  Quid::core c(validator.currency(), &pr, logger);
...
#define MAKE_NEXT_BLOCK_TX1(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, TX1)         \
  Quid::Block BLK_NAME;                                                         \
  {                                                                                   \
    std::list<Quid::Transaction> tx_list;                                       \
...
#define REWIND_BLOCKS_N(VEC_EVENTS, BLK_NAME, PREV_BLOCK, MINER_ACC, COUNT)           \
  Quid::Block BLK_NAME;                                                         \
  {                                                                                   \
    Quid::Block blk_last = PREV_BLOCK;                                          \
...
#define MAKE_TX_LIST_START(VEC_EVENTS, SET_NAME, FROM, TO, AMOUNT, HEAD) \
    std::list<Quid::Transaction> SET_NAME; \
...
const uint64_t COIN = Pow10<Quid::parameters::QUID_DISPLAY_DECIMAL_POINT>::value;
