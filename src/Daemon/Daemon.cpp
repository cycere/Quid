...
#include "QuidCore/MinerConfig.h"
...
namespace
{
  const command_line::arg_descriptor<std::string> arg_config_file = {"config-file", "Specify configuration file", std::string(Quid::QUID_NAME) + ".conf"};
...
void print_genesis_tx_hex() {
  ...
  Quid::Transaction tx = Quid::CurrencyBuilder(logger).generateGenesisTransaction();
  Quid::BinaryArray txb = Quid::toBinaryArray(tx);
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    bool r = command_line::handle_error_helper(desc_options, [&]()
    {
      ...
      {
        std::cout << Quid::QUID_NAME << " v" << PROJECT_VERSION_LONG << ENDL << ENDL;
    ...
    logger(INFO) << Quid::QUID_NAME << " v" << PROJECT_VERSION_LONG;
    ...
    Quid::CurrencyBuilder currencyBuilder(logManager);
    currencyBuilder.testnet(testnet_mode);

    try {
      currencyBuilder.currency();
    } catch (std::exception&) {
      std::cout << "GENESIS_COINBASE_TX_HEX constant has an incorrect value. Please launch: " << Quid::QUID_NAME << "d --" << arg_print_genesis_tx.name;
      return 1;
    }

    Quid::Currency currency = currencyBuilder.currency();
    Quid::core ccore(currency, nullptr, logManager);

    Quid::Checkpoints checkpoints(logManager);
    for (const auto& cp : Quid::CHECKPOINTS) {
    ...
    Quid::QuidProtocolHandler cprotocol(currency, dispatcher, ccore, nullptr, logManager);
    Quid::NodeServer p2psrv(dispatcher, cprotocol, logManager);
    Quid::RpcServer rpcServer(dispatcher, logManager, ccore, p2psrv, cprotocol);
...
bool command_line_preprocessor(const boost::program_options::variables_map &vm, LoggerRef &logger) {
  ...
  if (command_line::get_arg(vm, command_line::arg_version)) {
    std::cout << Quid::QUID_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    exit = true;
  }
