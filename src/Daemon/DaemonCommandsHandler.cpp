...
namespace {
  ...
  static bool print_as_json(const T& obj) {
    std::cout << Quid::storeToJson(obj) << ENDL;
    return true;
  }
}


DaemonCommandsHandler::DaemonCommandsHandler(Quid::core& core, Quid::NodeServer& srv, Logging::LoggerManager& log) :
...
{
  std::stringstream ss;
  ss << Quid::QUID_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
  ss << "Commands: " << ENDL;
