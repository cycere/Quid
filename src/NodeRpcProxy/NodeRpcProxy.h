...
class NodeRpcProxy : public Quid::INode {
public:
  ...
  virtual bool addObserver(Quid::INodeRpcProxyObserver* observer);
  virtual bool removeObserver(Quid::INodeRpcProxyObserver* observer);
...
private:
  ...
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>& knownBlockIds,
    std::vector<Quid::block_complete_entry>& newBlocks, uint32_t& startHeight);
...
private:
  ...
  Tools::ObserverManager<Quid::INodeObserver> m_observerManager;
  Tools::ObserverManager<Quid::INodeRpcProxyObserver> m_rpcProxyObserverManager;
