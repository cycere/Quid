...
InProcessNode::InProcessNode(Quid::ICore& core, Quid::IQuidProtocolQuery& protocol) :
...
void InProcessNode::init(const Callback& callback) {
  ...
  if (state != NOT_INITIALIZED) {
    ec = make_error_code(Quid::error::ALREADY_INITIALIZED);
  } else {
...
void InProcessNode::getNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<Quid::block_complete_entry>& newBlocks,
...
void InProcessNode::getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<Quid::block_complete_entry>& newBlocks,
  uint32_t& startHeight, const Callback& callback)
{
...
std::error_code InProcessNode::doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<Quid::block_complete_entry>& newBlocks, uint32_t& startHeight) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
  ...
  try {
    ...
    if (knownBlockIds.empty()) {
      return make_error_code(Quid::error::REQUEST_ERROR);
    }
    ...
    std::vector<Crypto::Hash> supplement = core.findBlockchainSupplement(knownBlockIds, Quid::COMMAND_RPC_GET_BLOCKS_FAST_MAX_COUNT, totalBlockCount, startHeight);
    ...
    return make_error_code(Quid::error::INTERNAL_NODE_ERROR);
  }
...
void InProcessNode::getRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
    std::vector<Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
...
//it's always protected with mutex
std::error_code InProcessNode::doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount, std::vector<Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
      return make_error_code(Quid::error::NOT_INITIALIZED);
  ...
  try {
    Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::response res;
    Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::request req;
    req.amounts = amounts;
...
void InProcessNode::relayTransaction(const Quid::Transaction& transaction, const Callback& callback)
{
  std::unique_lock<std::mutex> lock(mutex);
  if (state != INITIALIZED) {
    lock.unlock();
...
void InProcessNode::relayTransactionAsync(const Quid::Transaction& transaction, const Callback& callback) {
  std::error_code ec = doRelayTransaction(transaction);
  callback(ec);
}

//it's always protected with mutex
std::error_code InProcessNode::doRelayTransaction(const Quid::Transaction& transaction) {
  {
    std::unique_lock<std::mutex> lock(mutex);
    if (state != INITIALIZED) {
      return make_error_code(Quid::error::NOT_INITIALIZED);
  ...
  try {
    Quid::BinaryArray transactionBinaryArray = toBinaryArray(transaction);
    Quid::tx_verification_context tvc = boost::value_initialized<Quid::tx_verification_context>();
    ...
    Quid::NOTIFY_NEW_TRANSACTIONS::request r;
...
uint64_t InProcessNode::getLastLocalBlockTimestamp() const {
  ...
  if (!core.getBlockByHash(hash, block)) {
    throw std::system_error(make_error_code(Quid::error::INTERNAL_NODE_ERROR));
  }
...
std::error_code InProcessNode::doQueryBlocksLite(std::vector<Crypto::Hash>&& knownBlockIds, uint64_t timestamp, std::vector<BlockShortEntry>& newBlocks, uint32_t& startHeight) {
  ...
  std::vector<Quid::BlockShortInfo> entries;
