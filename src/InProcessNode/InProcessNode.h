...
class InProcessNode : public INode, public Quid::IQuidProtocolObserver, public Quid::ICoreObserver {
public:
  InProcessNode(Quid::ICore& core, Quid::IQuidProtocolQuery& protocol);
...
private:
  ...
  void getNewBlocksAsync(std::vector<Crypto::Hash>& knownBlockIds, std::vector<Quid::block_complete_entry>& newBlocks, uint32_t& startHeight, const Callback& callback);
  std::error_code doGetNewBlocks(std::vector<Crypto::Hash>&& knownBlockIds, std::vector<Quid::block_complete_entry>& newBlocks, uint32_t& startHeight);
  ...
  std::error_code doGetRandomOutsByAmounts(std::vector<uint64_t>&& amounts, uint64_t outsCount,
      std::vector<Quid::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount>& result);

  void relayTransactionAsync(const Quid::Transaction& transaction, const Callback& callback);
  std::error_code doRelayTransaction(const Quid::Transaction& transaction);
  ...
  State state;
  Quid::ICore& core;
  Quid::IQuidProtocolQuery& protocol;
