...
void wallet_rpc_server::processRequest(const Quid::HttpRequest& request, Quid::HttpResponse& response) {

  using namespace Quid::JsonRpc;
...
bool wallet_rpc_server::on_transfer(const wallet_rpc::COMMAND_RPC_TRANSFER::request& req, wallet_rpc::COMMAND_RPC_TRANSFER::response& res) {
  ...
  if (!req.payment_id.empty()) {
    ...
    if (!Quid::parsePaymentId(payment_id_str, payment_id)) {
    ...
    Quid::setPaymentIdToTransactionExtraNonce(extra_nonce, payment_id);
    if (!Quid::addExtraNonceToTransactionExtra(extra, extra_nonce)) {
      throw JsonRpc::JsonRpcError(WALLET_RPC_ERROR_CODE_WRONG_PAYMENT_ID,
  ...
  try {
    ...
    Quid::TransactionId tx = m_wallet.sendTransaction(transfers, req.fee, extraString, req.mixin, req.unlock_time);
...
bool wallet_rpc_server::on_get_payments(const wallet_rpc::COMMAND_RPC_GET_PAYMENTS::request& req, wallet_rpc::COMMAND_RPC_GET_PAYMENTS::response& res) {
  ...
  Quid::BinaryArray payment_id_blob;
