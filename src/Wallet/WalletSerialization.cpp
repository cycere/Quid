...
#include "QuidCore/QuidSerialization.h"
...
struct WalletTransactionDto {
  ...
  WalletTransactionDto(const Quid::WalletTransaction& wallet) {
    state = wallet.state;
  ...
  Quid::WalletTransactionState state;
  uint64_t timestamp;
  uint32_t blockHeight;
  Hash hash;
  int64_t totalAmount;
...
struct WalletTransferDto {
  ...
  WalletTransferDto(const Quid::WalletTransfer& tr, uint32_t version) : WalletTransferDto(version) {
    address = tr.address;
    amount = tr.amount;
...
void serialize(WalletRecordDto& value, Quid::ISerializer& serializer) {
...
void serialize(ObsoleteSpentOutputDto& value, Quid::ISerializer& serializer) {
...
void serialize(ObsoleteChangeDto& value, Quid::ISerializer& serializer) {
  serializer(value.txHash, "transaction_hash");
  serializer(value.amount, "amount");
}

void serialize(UnlockTransactionJobDto& value, Quid::ISerializer& serializer) {
  serializer(value.blockHeight, "block_height");
  serializer(value.transactionHash, "transaction_hash");
  serializer(value.walletIndex, "wallet_index");
}

void serialize(WalletTransactionDto& value, Quid::ISerializer& serializer) {
  typedef std::underlying_type<Quid::WalletTransactionState>::type StateType;

  StateType state = static_cast<StateType>(value.state);
  serializer(state, "state");
  value.state = static_cast<Quid::WalletTransactionState>(state);

  serializer(value.timestamp, "timestamp");
  Quid::serializeBlockHeight(serializer, value.blockHeight, "block_height");
...
void serialize(WalletTransferDto& value, Quid::ISerializer& serializer) {
...
std::string serialize(Object& obj, const std::string& name) {
  ...
  Quid::BinaryOutputStreamSerializer s(output);
...
std::string encrypt(const std::string& plain, Quid::CryptoContext& cryptoContext) {
  std::string cipher;
...
void addToStream(const std::string& cipher, const std::string& name, Common::IOutputStream& destination) {
  Quid::BinaryOutputStreamSerializer s(destination);
  s(const_cast<std::string& >(cipher), name);
}

template<typename Object>
void serializeEncrypted(Object& obj, const std::string& name, Quid::CryptoContext& cryptoContext, Common::IOutputStream& destination) {
...
std::string readCipher(Common::IInputStream& source, const std::string& name) {
  std::string cipher;
  Quid::BinaryInputStreamSerializer s(source);
  s(cipher, name);

  return cipher;
}

std::string decrypt(const std::string& cipher, Quid::CryptoContext& cryptoContext) {
  std::string plain;
...
void deserialize(Object& obj, const std::string& name, const std::string& plain) {
  ...
  Quid::BinaryInputStreamSerializer s(stream);
  s(obj, Common::StringView(name));
}

template<typename Object>
void deserializeEncrypted(Object& obj, const std::string& name, Quid::CryptoContext& cryptoContext, Common::IInputStream& source) {
...
Quid::WalletTransaction convert(const Quid::WalletLegacyTransaction& tx) {
  Quid::WalletTransaction mtx;

  mtx.state = Quid::WalletTransactionState::SUCCEEDED;
...
Quid::WalletTransfer convert(const Quid::WalletLegacyTransfer& tr) {
  Quid::WalletTransfer mtr;
...
void WalletSerializer::loadWallet(Common::IInputStream& source, const std::string& password, uint32_t version) {
  Quid::CryptoContext cryptoContext;
...
void WalletSerializer::loadWalletV1(Common::IInputStream& source, const std::string& password) {
  ...
  Quid::BinaryInputStreamSerializer encrypted(source);
  ...
  Quid::BinaryInputStreamSerializer serializer(decryptedStream);
...
void WalletSerializer::loadWalletV1Keys(Quid::BinaryInputStreamSerializer& serializer) {
  Quid::KeysStorage keys;
...
void WalletSerializer::loadWalletV1Details(Quid::BinaryInputStreamSerializer& serializer) {
...
void WalletSerializer::loadWallets(Common::IInputStream& source, CryptoContext& cryptoContext) {
  ...
  for (uint64_t i = 0; i < count; ++i) {
    ...
    wallet.container = reinterpret_cast<Quid::ITransfersContainer*>(i); //dirty hack. container field must be unique
