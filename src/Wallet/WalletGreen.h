...
protected:
  ...
  struct ReceiverAmounts {
    Quid::AccountPublicAddress receiver;
  ...
  void updateBalance(Quid::ITransfersContainer* container);
  ...
  const WalletRecord& getWalletRecord(Quid::ITransfersContainer* container) const;

  Quid::AccountPublicAddress parseAddress(const std::string& address) const;
  ...
  void prepareTransaction(std::vector<WalletOuts>&& wallets,
    ...
    const Quid::AccountPublicAddress& changeDestinationAddress,
  ...
  std::unique_ptr<Quid::ITransaction> makeTransaction(const std::vector<ReceiverAmounts>& decomposedOutputs,
    std::vector<InputInfo>& keysInfo, const std::string& extra, uint64_t unlockTimestamp);

  void sendTransaction(const Quid::Transaction& cryptoNoteTransaction);
  ...
  bool updateWalletTransactionInfo(size_t transactionId, const Quid::TransactionInformation& info, int64_t totalAmount);
  ...
  void insertUnlockTransactionJob(const Crypto::Hash& transactionHash, uint32_t blockHeight, Quid::ITransfersContainer* container);
  ...
  Quid::AccountPublicAddress getChangeDestination(const std::string& changeDestinationAddress, const std::vector<std::string>& sourceAddresses) const;
