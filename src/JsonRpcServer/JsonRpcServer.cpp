...
void JsonRpcServer::processRequest(const Quid::HttpRequest& req, Quid::HttpResponse& resp) {
  try {
    ...
    if (req.getUrl() == "/json_rpc") {
      ...
      try {
        ...
        resp.setStatus(Quid::HttpResponse::STATUS_200);
      ...
      resp.setStatus(Quid::HttpResponse::STATUS_404);
      return;
    }
  } catch (std::exception& e) {
    logger(Logging::WARNING) << "Error while processing http request: " << e.what();
    resp.setStatus(Quid::HttpResponse::STATUS_500);
  }
}
