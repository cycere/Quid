...
#include "QuidCore/OnceInInterval.h"
...
namespace Quid
{
  ...
  struct P2pConnectionContext : public QuidConnectionContext {
  public:
    ...
    P2pConnectionContext(P2pConnectionContext&& ctx) : 
      QuidConnectionContext(std::move(ctx)),
      context(ctx.context),
      peerId(ctx.peerId),
  ...
  public:
    ...
    NodeServer(System::Dispatcher& dispatcher, Quid::QuidProtocolHandler& payload_handler, Logging::ILogger& log);
    ...
    Quid::QuidProtocolHandler& get_payload_object();
    ...
    Quid::PeerlistManager& getPeerlistManager() { return m_peerlist; }
  ...
  private:
    ...
    bool handshake(Quid::LevinProtocol& proto, P2pConnectionContext& context, bool just_take_peerlist = false);
    bool timedSync();
    ...
    virtual bool invoke_notify_to_peer(int command, const BinaryArray& req_buff, const QuidConnectionContext& context) override;
    ...
    bool idle_worker();
    bool handle_remote_peerlist(const std::list<PeerlistEntry>& peerlist, time_t local_time, const QuidConnectionContext& context);
    ...
    QuidProtocolHandler& m_payload_handler;
