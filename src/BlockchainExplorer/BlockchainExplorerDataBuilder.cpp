...
BlockchainExplorerDataBuilder::BlockchainExplorerDataBuilder(Quid::ICore& core, Quid::IQuidProtocolQuery& protocol) :
core(core),
protocol(protocol) {
}
...
bool BlockchainExplorerDataBuilder::fillBlockDetails(const Block &block, BlockDetails& blockDetails) {
  ...
  if (!core.getBackwardBlocksSizes(blockDetails.height, blocksSizes, parameters::QUID_REWARD_BLOCKS_WINDOW)) {
    return false;
  }
