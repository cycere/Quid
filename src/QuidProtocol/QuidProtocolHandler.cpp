...
#include "QuidCore/VerificationContext.h"
...
bool post_notify(IP2pEndpoint& p2p, typename t_parametr::request& arg, const QuidConnectionContext& context) {
...
QuidProtocolHandler::QuidProtocolHandler(const Currency& currency, System::Dispatcher& dispatcher, ICore& rcore, IP2pEndpoint* p_net_layout, Logging::ILogger& log) :
...
size_t QuidProtocolHandler::getPeerCount() const {
  return m_peersCount;
}

void QuidProtocolHandler::set_p2p_endpoint(IP2pEndpoint* p2p) {
  if (p2p)
    m_p2p = p2p;
  else
    m_p2p = &m_p2p_stub;
...
void QuidProtocolHandler::onConnectionOpened(QuidConnectionContext& context) {
}

void QuidProtocolHandler::onConnectionClosed(QuidConnectionContext& context) {
  bool updated = false;
  {
  ...
  if (updated) {
    ...
    m_observerManager.notify(&IQuidProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
  }

  if (context.m_state != QuidConnectionContext::state_befor_handshake) {
    m_peersCount--;
    m_observerManager.notify(&IQuidProtocolObserver::peerCountUpdated, m_peersCount.load());
  }
}

void QuidProtocolHandler::stop() {
  m_stop = true;
}

bool QuidProtocolHandler::start_sync(QuidConnectionContext& context) {
  logger(Logging::TRACE) << context << "Starting synchronization";

  if (context.m_state == QuidConnectionContext::state_synchronizing) {
...
bool QuidProtocolHandler::get_stat_info(core_stat_info& stat_inf) {
  return m_core.get_stat_info(stat_inf);
}

void QuidProtocolHandler::log_connections() {
  std::stringstream ss;
  ...
  m_p2p->for_each_connection([&](const QuidConnectionContext& cntxt, PeerIdType peer_id) {
...
uint32_t QuidProtocolHandler::get_current_blockchain_height() {
  uint32_t height;
  Crypto::Hash blockId;
...
bool QuidProtocolHandler::process_payload_sync_data(const CORE_SYNC_DATA& hshd, QuidConnectionContext& context, bool is_inital) {
  if (context.m_state == QuidConnectionContext::state_befor_handshake && !is_inital)
    return true;
  ...
  if (context.m_state == QuidConnectionContext::state_synchronizing) {
    ...
    if (is_inital) {
      ...
      context.m_state = QuidConnectionContext::state_pool_sync_required;
    } else {
      context.m_state = QuidConnectionContext::state_normal;
    }
  } else {
    ...
    context.m_state = QuidConnectionContext::state_sync_required;
  }
...
bool QuidProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA& hshd) {
...
int notifyAdaptor(const BinaryArray& reqBuf, QuidConnectionContext& ctx, Handler handler) {
...
int QuidProtocolHandler::handleCommand(bool is_notify, int command, const BinaryArray& in, BinaryArray& out, QuidConnectionContext& ctx, bool& handled) {
  int ret = 0;
  handled = true;

  switch (command) {
    HANDLE_NOTIFY(NOTIFY_NEW_BLOCK, &QuidProtocolHandler::handle_notify_new_block)
    HANDLE_NOTIFY(NOTIFY_NEW_TRANSACTIONS, &QuidProtocolHandler::handle_notify_new_transactions)
    HANDLE_NOTIFY(NOTIFY_REQUEST_GET_OBJECTS, &QuidProtocolHandler::handle_request_get_objects)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_GET_OBJECTS, &QuidProtocolHandler::handle_response_get_objects)
    HANDLE_NOTIFY(NOTIFY_REQUEST_CHAIN, &QuidProtocolHandler::handle_request_chain)
    HANDLE_NOTIFY(NOTIFY_RESPONSE_CHAIN_ENTRY, &QuidProtocolHandler::handle_response_chain_entry)
    HANDLE_NOTIFY(NOTIFY_REQUEST_TX_POOL, &QuidProtocolHandler::handleRequestTxPool)
...
int QuidProtocolHandler::handle_notify_new_block(int command, NOTIFY_NEW_BLOCK::request& arg, QuidConnectionContext& context) {
  ...
  if (context.m_state != QuidConnectionContext::state_normal) {
    return 1;
  }

  for (auto tx_blob_it = arg.b.txs.begin(); tx_blob_it != arg.b.txs.end(); tx_blob_it++) {
    Quid::tx_verification_context tvc = boost::value_initialized<decltype(tvc)>();
    ...
    if (tvc.m_verifivation_failed) {
      ...
      context.m_state = QuidConnectionContext::state_shutdown;
      return 1;
    }
  }
  ...
  if (bvc.m_added_to_main_chain) {
    ...
    context.m_state = QuidConnectionContext::state_synchronizing;
    NOTIFY_REQUEST_CHAIN::request r = boost::value_initialized<NOTIFY_REQUEST_CHAIN::request>();
    r.block_ids = m_core.buildSparseChain();
    logger(Logging::TRACE) << context << "-->>NOTIFY_REQUEST_CHAIN: m_block_ids.size()=" << r.block_ids.size();
    post_notify<NOTIFY_REQUEST_CHAIN>(*m_p2p, r, context);
...
int QuidProtocolHandler::handle_notify_new_transactions(int command, NOTIFY_NEW_TRANSACTIONS::request& arg, QuidConnectionContext& context) {
...
int QuidProtocolHandler::handle_request_get_objects(int command, NOTIFY_REQUEST_GET_OBJECTS::request& arg, QuidConnectionContext& context) {
...
int QuidProtocolHandler::handle_response_get_objects(int command, NOTIFY_RESPONSE_GET_OBJECTS::request& arg, QuidConnectionContext& context) {
  ...
  for (const block_complete_entry& block_entry : arg.blocks) {
    ...
    if (count == 2) {
      if (m_core.have_block(get_block_hash(b))) {
        context.m_state = QuidConnectionContext::state_idle;
  ...
  if (!m_stop && context.m_state == QuidConnectionContext::state_synchronizing) {
...
int QuidProtocolHandler::processObjects(QuidConnectionContext& context, const std::vector<block_complete_entry>& blocks) {
...
bool QuidProtocolHandler::on_idle() {
  return m_core.on_idle();
}

int QuidProtocolHandler::handle_request_chain(int command, NOTIFY_REQUEST_CHAIN::request& arg, QuidConnectionContext& context) {
...
bool QuidProtocolHandler::request_missing_objects(QuidConnectionContext& context, bool check_having_blocks) {
...
bool QuidProtocolHandler::on_connection_synchronized() {
  ...
  if (m_synchronized.compare_exchange_strong(val_expected, true)) {
    ...
    m_observerManager.notify(&IQuidProtocolObserver::blockchainSynchronized, height);
  }
  return true;
}

int QuidProtocolHandler::handle_response_chain_entry(int command, NOTIFY_RESPONSE_CHAIN_ENTRY::request& arg, QuidConnectionContext& context) {
...
int QuidProtocolHandler::handleRequestTxPool(int command, NOTIFY_REQUEST_TX_POOL::request& arg,
                                                     QuidConnectionContext& context) {
...
void QuidProtocolHandler::relay_block(NOTIFY_NEW_BLOCK::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_BLOCK::ID, buf);
}

void QuidProtocolHandler::relay_transactions(NOTIFY_NEW_TRANSACTIONS::request& arg) {
  auto buf = LevinProtocol::encode(arg);
  m_p2p->externalRelayNotifyToAll(NOTIFY_NEW_TRANSACTIONS::ID, buf);
}

void QuidProtocolHandler::requestMissingPoolTransactions(const QuidConnectionContext& context) {
...
void QuidProtocolHandler::updateObservedHeight(uint32_t peerHeight, const QuidConnectionContext& context) {
  bool updated = false;
  {
    std::lock_guard<std::mutex> lock(m_observedHeightMutex);
...
void QuidProtocolHandler::recalculateMaxObservedHeight(const QuidConnectionContext& context) {
  //should be locked outside
  uint32_t peerHeight = 0;
  m_p2p->for_each_connection([&peerHeight, &context](const QuidConnectionContext& ctx, PeerIdType peerId) {
...
uint32_t QuidProtocolHandler::getObservedHeight() const {
  std::lock_guard<std::mutex> lock(m_observedHeightMutex);
  return m_observedHeight;
};

bool QuidProtocolHandler::addObserver(IQuidProtocolObserver* observer) {
  return m_observerManager.add(observer);
}

bool QuidProtocolHandler::removeObserver(IQuidProtocolObserver* observer) {
