...
bool Currency::parseAccountAddressString(const std::string& str, AccountPublicAddress& addr) const {
  uint64_t prefix;
  if (!Quid::parseAccountAddressString(prefix, addr, str)) {
    return false;
  }
...
CurrencyBuilder::CurrencyBuilder(Logging::ILogger& log) : m_currency(log) {
  maxBlockNumber(parameters::QUID_MAX_BLOCK_NUMBER);
  maxBlockBlobSize(parameters::QUID_MAX_BLOCK_BLOB_SIZE);
  maxTxSize(parameters::QUID_MAX_TX_SIZE);
  publicAddressBase58Prefix(parameters::QUID_PUBLIC_ADDRESS_BASE58_PREFIX);
  minedMoneyUnlockWindow(parameters::QUID_MINED_MONEY_UNLOCK_WINDOW);

  timestampCheckWindow(parameters::BLOCKCHAIN_TIMESTAMP_CHECK_WINDOW);
  blockFutureTimeLimit(parameters::QUID_BLOCK_FUTURE_TIME_LIMIT);

  moneySupply(parameters::MONEY_SUPPLY);
  emissionSpeedFactor(parameters::EMISSION_SPEED_FACTOR);

  rewardBlocksWindow(parameters::QUID_REWARD_BLOCKS_WINDOW);
  blockGrantedFullRewardZone(parameters::QUID_BLOCK_GRANTED_FULL_REWARD_ZONE);
  minerTxBlobReservedSize(parameters::QUID_COINBASE_BLOB_RESERVED_SIZE);

  numberOfDecimalPlaces(parameters::QUID_DISPLAY_DECIMAL_POINT);
  ...
  lockedTxAllowedDeltaSeconds(parameters::QUID_LOCKED_TX_ALLOWED_DELTA_SECONDS);
  lockedTxAllowedDeltaBlocks(parameters::QUID_LOCKED_TX_ALLOWED_DELTA_BLOCKS);

  mempoolTxLiveTime(parameters::QUID_MEMPOOL_TX_LIVETIME);
  mempoolTxFromAltBlockLiveTime(parameters::QUID_MEMPOOL_TX_FROM_ALT_BLOCK_LIVETIME);
  numberOfPeriodsToForgetTxDeletedFromPool(parameters::QUID_NUMBER_OF_PERIODS_TO_FORGET_TX_DELETED_FROM_POOL);

  fusionTxMaxSize(parameters::FUSION_TX_MAX_SIZE);
  fusionTxMinInputCount(parameters::FUSION_TX_MIN_INPUT_COUNT);
  fusionTxMinInOutCountRatio(parameters::FUSION_TX_MIN_IN_OUT_COUNT_RATIO);

  blocksFileName(parameters::QUID_BLOCKS_FILENAME);
  blocksCacheFileName(parameters::QUID_BLOCKSCACHE_FILENAME);
  blockIndexesFileName(parameters::QUID_BLOCKINDEXES_FILENAME);
  txPoolFileName(parameters::QUID_POOLDATA_FILENAME);
  blockchinIndicesFileName(parameters::QUID_BLOCKCHAIN_INDICES_FILENAME);
...
Transaction CurrencyBuilder::generateGenesisTransaction() {
  Quid::Transaction tx;
  Quid::AccountPublicAddress ac = boost::value_initialized<Quid::AccountPublicAddress>();
