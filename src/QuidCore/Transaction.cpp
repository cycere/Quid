...
namespace Quid {
  ...
  public:
    ...
    TransactionImpl(const Quid::Transaction& tx);
  ...
  std::unique_ptr<ITransaction> createTransaction(const Quid::Transaction& tx) {
    return std::unique_ptr<ITransaction>(new TransactionImpl(tx));
  }

  TransactionImpl::TransactionImpl() {   
    Quid::KeyPair txKeys(Quid::generateKeyPair());
  ...
  TransactionImpl::TransactionImpl(const Quid::Transaction& tx) : transaction(tx) {
    extra.parse(transaction.extra);
  }
  ...
  bool TransactionImpl::findOutputsToAccount(const AccountPublicAddress& addr, const SecretKey& viewSecretKey, std::vector<uint32_t>& out, uint64_t& amount) const {
    return ::Quid::findOutputsToAccount(transaction, addr, viewSecretKey, out, amount);
  }
