...
struct BinaryVariantTagGetter: boost::static_visitor<uint8_t> {
  uint8_t operator()(const Quid::BaseInput) { return  0xff; }
  uint8_t operator()(const Quid::KeyInput) { return  0x2; }
  uint8_t operator()(const Quid::MultisignatureInput) { return  0x3; }
  uint8_t operator()(const Quid::KeyOutput) { return  0x2; }
  uint8_t operator()(const Quid::MultisignatureOutput) { return  0x3; }
  uint8_t operator()(const Quid::Transaction) { return  0xcc; }
  uint8_t operator()(const Quid::Block) { return  0xbb; }
};

struct VariantSerializer : boost::static_visitor<> {
  VariantSerializer(Quid::ISerializer& serializer, const std::string& name) : s(serializer), name(name) {}
...
void getVariantValue(Quid::ISerializer& serializer, uint8_t tag, Quid::TransactionInput& in) {
  switch(tag) {
  case 0xff: {
    Quid::BaseInput v;
    serializer(v, "value");
...
void getVariantValue(Quid::ISerializer& serializer, uint8_t tag, Quid::TransactionOutputTarget& out) {
  switch(tag) {
  case 0x2: {
    Quid::KeyOutput v;
    serializer(v, "data");
...
template <typename T>
bool serializePod(T& v, Common::StringView name, Quid::ISerializer& serializer) {
  return serializer.binary(&v, sizeof(v), name);
}

bool serializeVarintVector(std::vector<uint32_t>& vector, Quid::ISerializer& serializer, Common::StringView name) {
...
bool serialize(PublicKey& pubKey, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(pubKey, name, serializer);
}

bool serialize(SecretKey& secKey, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(secKey, name, serializer);
}

bool serialize(Hash& h, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(h, name, serializer);
}

bool serialize(KeyImage& keyImage, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(keyImage, name, serializer);
}

bool serialize(chacha8_iv& chacha, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(chacha, name, serializer);
}

bool serialize(Signature& sig, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(sig, name, serializer);
}

bool serialize(EllipticCurveScalar& ecScalar, Common::StringView name, Quid::ISerializer& serializer) {
  return serializePod(ecScalar, name, serializer);
}

bool serialize(EllipticCurvePoint& ecPoint, Common::StringView name, Quid::ISerializer& serializer) {
