...
const Quid::Currency PaymentGateService::getCurrency() {
...
void PaymentGateService::runInProcess(Logging::LoggerRef& log) {
  ...
  Quid::core core(currency, NULL, logger);

  Quid::QuidProtocolHandler protocol(currency, *dispatcher, core, NULL, logger);
  Quid::NodeServer p2pNode(*dispatcher, protocol, logger);
  ...
  std::unique_ptr<Quid::INode> node(new Quid::InProcessNode(core, protocol));
...
void PaymentGateService::runRpcProxy(Logging::LoggerRef& log) {
  ...
  std::unique_ptr<Quid::INode> node(
...
void PaymentGateService::runWalletService(const Quid::Currency& currency, Quid::INode& node) {
  PaymentService::WalletConfiguration walletConfiguration{
    config.gateConfiguration.containerFile,
    config.gateConfiguration.containerPassword
  };

  std::unique_ptr<Quid::IWallet> wallet (WalletFactory::createWallet(currency, node, *dispatcher));
