...
std::error_code walletSaveWrapper(Quid::IWalletLegacy& wallet, std::ofstream& file, bool saveDetailes, bool saveCache) {
  Quid::WalletHelper::SaveWalletResultObserver o;
...
void WalletHelper::SendCompleteResultObserver::sendTransactionCompleted(Quid::TransactionId transactionId, std::error_code result) {
  std::lock_guard<std::mutex> lock(m_mutex);
  m_finishedTransactions[transactionId] = result;
  m_condition.notify_one();
}

std::error_code WalletHelper::SendCompleteResultObserver::wait(Quid::TransactionId transactionId) {
...
WalletHelper::IWalletRemoveObserverGuard::IWalletRemoveObserverGuard(Quid::IWalletLegacy& wallet, Quid::IWalletLegacyObserver& observer) :
  m_wallet(wallet),
  m_observer(observer),
  m_removed(false) {
...
void WalletHelper::storeWallet(Quid::IWalletLegacy& wallet, const std::string& walletFilename) {
