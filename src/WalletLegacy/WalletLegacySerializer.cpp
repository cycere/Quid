...
WalletLegacySerializer::WalletLegacySerializer(Quid::AccountBase& account, WalletUserTransactionsCache& transactionsCache) :
  account(account),
...
void WalletLegacySerializer::serialize(std::ostream& stream, const std::string& password, bool saveDetailed, const std::string& cache) {
  ...
  Quid::BinaryOutputStreamSerializer serializer(plainStream);
  saveKeys(serializer);
...
void WalletLegacySerializer::saveKeys(Quid::ISerializer& serializer) {
  Quid::KeysStorage keys;
  Quid::AccountKeys acc = account.getAccountKeys();
...
void WalletLegacySerializer::deserialize(std::istream& stream, const std::string& password, std::string& cache) {
  ...
  Quid::BinaryInputStreamSerializer serializerEncrypted(stdStream);
...
void WalletLegacySerializer::loadKeys(Quid::ISerializer& serializer) {
  Quid::KeysStorage keys;

  keys.serialize(serializer, "keys");

  Quid::AccountKeys acc;
