...
template<> 
struct hash<Quid::TransactionOutputId> {
  size_t operator()(const Quid::TransactionOutputId &_v) const {    
...
public:
  ...
  bool serialize(Quid::ISerializer& s);

  bool findTransactionId(const Crypto::Hash& hash, TransactionId& id);
  void erase(const Crypto::Hash& hash);
  void add(const Quid::Transaction& tx, TransactionId transactionId, 
