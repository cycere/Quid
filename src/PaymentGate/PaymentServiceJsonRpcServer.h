...
class PaymentServiceJsonRpcServer : public Quid::JsonRpcServer {
public:
...
private:
  ...
  HandlerFunction jsonHandler(RequestHandler handler) {
    return [handler] (const Common::JsonValue& jsonRpcParams, Common::JsonValue& jsonResponse) mutable {
      ...
      try {
        Quid::JsonInputValueSerializer inputSerializer(const_cast<Common::JsonValue&>(jsonRpcParams));
      ...
      Quid::JsonOutputStreamSerializer outputSerializer;
