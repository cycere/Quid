...
class WalletService {
public:
  WalletService(const Quid::Currency& currency, System::Dispatcher& sys, Quid::INode& node, Quid::IWallet& wallet, const WalletConfiguration& conf, Logging::ILogger& logger);
...
private:
  ...
  std::vector<Quid::TransactionsInBlockInfo> getTransactions(const Crypto::Hash& blockHash, size_t blockCount) const;
  std::vector<Quid::TransactionsInBlockInfo> getTransactions(uint32_t firstBlockIndex, size_t blockCount) const;
