...
std::vector<Crypto::Hash> getBlockHashes(const Quid::CompleteBlock* blocks, size_t count) {
...
TransfersConsumer::TransfersConsumer(const Quid::Currency& currency, INode& node, const SecretKey& viewSecret) :
...
std::error_code createTransfers(
  ...
  for (auto idx : outputs) {
    ...
    if (outType == TransactionTypes::OutputType::Key) {
      ...
      Quid::generate_key_image_helper(
        account,
        txPubKey,
        idx,
        in_ephemeral,
