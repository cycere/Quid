...
const char* getStatusString(Quid::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case Quid::HttpResponse::STATUS_200:
    return "200 OK";
  case Quid::HttpResponse::STATUS_404:
    return "404 Not Found";
  case Quid::HttpResponse::STATUS_500:
...
const char* getErrorBody(Quid::HttpResponse::HTTP_STATUS status) {
  switch (status) {
  case Quid::HttpResponse::STATUS_404:
...
HttpResponse::HttpResponse() {
  status = STATUS_200;
  headers["Server"] = "Quid-based HTTP server";
}
