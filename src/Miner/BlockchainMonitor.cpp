...
Crypto::Hash BlockchainMonitor::requestLastBlockHash() {
  ...
  try {
    Quid::HttpClient client(m_dispatcher, m_daemonHost, m_daemonPort);

    Quid::COMMAND_RPC_GET_LAST_BLOCK_HEADER::request request;
    Quid::COMMAND_RPC_GET_LAST_BLOCK_HEADER::response response;

    System::EventLock lk(m_httpEvent);
    Quid::JsonRpc::invokeJsonRpcCommand(client, "getlastblockheader", request, response);
