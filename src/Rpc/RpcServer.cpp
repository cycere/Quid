...
RpcServer::RpcServer(System::Dispatcher& dispatcher, Logging::ILogger& log, core& c, NodeServer& p2p, const IQuidProtocolQuery& protocolQuery) :
...
bool RpcServer::onGetPoolChanges(const COMMAND_RPC_GET_POOL_CHANGES::request& req, COMMAND_RPC_GET_POOL_CHANGES::response& rsp) {
  ...
  std::vector<Quid::Transaction> addedTransactions;
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  ...
  PublicKey tx_pub_key = Quid::getTransactionPublicKeyFromExtra(b.baseTransaction.extra);
